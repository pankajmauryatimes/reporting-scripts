...
where ld.isFakeEmail = 0
 ...
 and ld.last_modified_date >= (select date_key from date_dimension where full_date= CURRENT_date() - inTERVAL 24 month)
  and ld.mail_primary is not null
  and ld.phone_mobile is not null
 order by ld.last_modified_date desc
 ) tt  where tt.cand_name is not null INTO OUTFILE '/tmp/promotional_data_F21_${ddate}.txt' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\\n'; " > $HOME/generic_etl/sql_scripts/promotional_data_script_f21.sql
