...
then
	...
	mail -s "$cur_proc_name - FAILED with PWHS File - Execution Errors" $email < ${LOG_DIR}/${LOG_FILE}
	exit 1
fi
...
then
  ...
  then
     ...
     printf "Convert date format in ORIG_FILE_NAME to actual date.\n" >> ${LOG_DIR}/${LOG_FILE}	
       ...
       mail -s "$cur_proc_name - WARNING" $email < ${LOG_DIR}/${LOG_FILE}
       exit 1
  fi
...
if [ -r ${KSH_SCRIPTS_DIR}/pre_${cur_proc}.ksh ]
    ...
    if [[ $exit_code != 0 ]]
       ...
       mail -s "$cur_proc_name - FAILED with PRE KSH Script" $email < ${LOG_DIR}/${LOG_FILE}
       exit 1
   fi
fi
...
if [[ $time < $cutoff ]]; then
  if [[ $LOAD_FILE != "" ]]; then
     ...
     while [[ $time < $cutoff ]]; do
	   ...
	   mail -s "$cur_proc - WARNING DB is DOWN" $email < ${LOG_DIR}/${LOG_FILE}
         fi
     done
     ...
     if [[ $time > $cutoff ]]; then
             ...
             then
                mail -s "WARNING - $cur_proc failed as the DATABASE IS DOWN" $email < ${LOG_DIR}/${LOG_FILE}
             fi
             printf "Process timed out before completion!\n" >> ${LOG_DIR}/${LOG_FILE}
             printf "TAKE CORRECTIVE ACTION IMMEDIATELY!!!\n" >> ${LOG_DIR}/${LOG_FILE}
             time=$(date +"%T")
             ...
             exit 2
               ...
               if [ -r ${CTL_FILE} ]
                 ...
                 if [[ $exit_code == 127 ]] || [[ $exit_code == 3 ]] || [[ $exit_code == 1 ]]
                    ...
                    mail -s "$cur_proc_name - FAILED with SQL Loader - Syntax Errors" $email < ${LOG_DIR}/${LOG_FILE}
                    exit 1
                 fi
	       ...
	       if [[ $time < $cutoff ]]; then
	  	    ...
	  	    then
	  	           ...
	  	           then
	  	               ...
	  	               mail -s "$cur_proc_name - FAILED with SQL Script" $email < ${LOG_DIR}/${LOG_FILE}
	  	               exit 1
	  	           fi
	  	    fi
	  	    printf "Processed SQL Script ...\n" >> ${LOG_DIR}/${LOG_FILE}
                    ...
                    if [[ ${ktl_jobarr[$i]} != "" ]]; then
                         ...
                         while [[ ${i} -lt $ktl_totaljobs ]]; do
                                ...
                                if [[ $exit_code != 0 ]]
                                then
                                    printf "KETTLE Script Processed. Please see log file for more information.\n" >> ${LOG_DIR}/${LOG_FILE}
                                    printf "Error: Error in KETTLE Script Execution. EXIT Code = $exit_code \n" >> ${LOG_DIR}/${LOG_FILE}
                                    mail -s "$cur_proc_name - FAILED with KETTLE Script" $email < ${LOG_DIR}/${LOG_FILE}
                                    exit 1
                                fi
                                i=`expr $i + 1`
                         done
		    ...
		    then
		      ...
		      mail -s "$cur_proc_name - WARNING :Syntax Errors" $email < ${LOG_DIR}/${LOG_FILE}
		      exit 1
		   fi
	       fi
     fi
