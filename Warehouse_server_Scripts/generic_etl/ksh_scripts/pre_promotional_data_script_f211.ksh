...
where ld.isFakeEmail = 0
  ...
  and rd.work_exp between 7 and 9
 and  exists (select 1 from date_dimension dd where dd.full_date between current_date() - inTERVAL 36 month and current_date - interval 24 month and ld.last_modified_date = dd.date_key)
  and ld.mail_primary is not null
  and ld.phone_mobile is not null
 order by ld.last_modified_date desc
 ) tt  where tt.cand_name is not null INTO OUTFILE '/tmp/promotional_data_F211_${ddate}.txt' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\\n'; " > $HOME/generic_etl/sql_scripts/promotional_data_script_f211.sql
