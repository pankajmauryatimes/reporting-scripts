...
then
	...
	/home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - FAILED with PWHS File - Execution Errors" < ${LOG_DIR}/${LOG_FILE}
	exit 1
fi

printf $(date) >> ${LOG_DIR}/${LOG_FILE} >> ${LOG_DIR}/${LOG_FILE}
...
if [[ $time > $cutoff ]]; then
   ...
   /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc - WARNING " < ${LOG_DIR}/${LOG_FILE}
   exit 1
fi
...
then
  ...
  then
     ...
     printf "Convert date format in ORIG_FILE_NAME to actual date.\n" >> ${LOG_DIR}/${LOG_FILE}	
       ...
       /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - WARNING" < ${LOG_DIR}/${LOG_FILE}
       exit 1
  fi
...
if [ -r ${KSH_SCRIPTS_DIR}/pre_${cur_proc}.ksh ]
    ...
    if [[ $exit_code != 0 ]]
       ...
       /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - FAILED with PRE KSH Script" < ${LOG_DIR}/${LOG_FILE}
       exit 1
   fi
fi
...
if [[ $time < $cutoff ]]; then
  if [[ $LOAD_FILE != "" ]]; then
     ...
     while [[ $time < $cutoff ]]; do
	   ...
	   /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc - WARNING DB is DOWN" < ${LOG_DIR}/${LOG_FILE}
         fi
     done
     ...
     if [[ $time > $cutoff ]]; then
             ...
             then
                /home/datareq/generic_etl/ksh_scripts/mail2 $email "WARNING - $cur_proc failed as the DATABASE IS DOWN" < ${LOG_DIR}/${LOG_FILE}
             fi
             printf "Process timed out before completion!\n" >> ${LOG_DIR}/${LOG_FILE}
             printf "TAKE CORRECTIVE ACTION IMMEDIATELY!!!\n" >> ${LOG_DIR}/${LOG_FILE}
             time=$(date +"%T")
             ...
             exit 2
               ...
               if [ -r ${CTL_FILE} ]
                 ...
                 exit_code=$? 
                 if [[ $exit_code == 127 ]] || [[ $exit_code == 3 ]]
                    then
                    time=$(date +"%T")
                    printf "time is "$time >> ${LOG_DIR}/${LOG_FILE}
                    printf "\nError: Error in SQL Load Script Execution with Syntax errors. EXIT Code = $exit_code \n" >> ${LOG_DIR}/${LOG_FILE}
                    /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - FAILED with SQL Loader - Syntax Errors" < ${LOG_DIR}/${LOG_FILE}
                    exit 1
                 fi
	       ...
	       if [[ $time < $cutoff ]]; then
	  	    ...
	  	    then
	  	           ...
	  	           then
	  	               ...
	  	               /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - FAILED with SQL Script" < ${LOG_DIR}/${LOG_FILE}
	  	               exit 1
	  	           fi
	  	    fi
	  	     ...
	  	     then
	  	          ...
	  	          then
	  	             ...
	  	             /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - FAILED with POST KSH Script" < ${LOG_DIR}/${LOG_FILE}
	  	             exit 1
	  	          fi
	             fi
		    ...
		    then
		      ...
		      /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name - WARNING :Syntax Errors" < ${LOG_DIR}/${LOG_FILE}
		      exit 1
		   fi
	       fi
     fi
  ...
  if [ "$send_mail" == "Y" ]; then
     ...
     /home/datareq/generic_etl/ksh_scripts/mail2 $email "$cur_proc_name Process Finished " < $HOME/generic_etl/logs/dummy.txt
     echo " " >$HOME/generic_etl/logs/dummy.txt
  fi
  touch /home/datareq/generic_etl/job_complete/${cur_proc}.done
fi
