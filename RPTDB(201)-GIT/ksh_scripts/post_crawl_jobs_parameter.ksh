...
if [ -r $LOAD_DIR crawl_jobs_parameter.csv ]
    then
    printf "\nFile $LOAD_DIR/crawl_jobs_parameter.csv ...\n"
    printf "\nFile $LOAD_DIR/crawl_jobs_parameter.csv ...\n" >> ${LOG_DIR}/${LOG_FILE}
else
       printf "File $LOAD_DIR/crawl_jobs_parameter.csv do not found.\n" >> ${LOG_DIR}/${LOG_FILE}
...
mv $LOAD_DIR/crawl_jobs_parameter.csv $LOAD_DIR/crawl_jobs_parameter_${todate}.csv
...
echo "pwd" >> $ftp_file
echo "put crawl_jobs_parameter_${todate}.csv" >> $ftp_file
echo "quit" >> $ftp_file
echo "EOF" >> $ftp_file
ftp_file=${LOAD_DIR}/${cur_proc}.ftp
#execute FTP script
