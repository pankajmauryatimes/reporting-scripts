...
if [[ $time < $cutoff ]]; then
                    ...
                    if [[ ${ktl_jobarr[$i]} != "" ]]; then
                         ...
                         while [[ ${i} -lt $ktl_totaljobs ]]; do
                                ...
                                then
                                    ...
                                    ${KSH_SCRIPTS_DIR}/mail $email   "$cur_proc_name - FAILED with KETTLE Script"  ${LOG_DIR}/${LOG_FILE}
                                    exit 1
                                fi
                                i=`expr $i + 1`
                         done
  ...
  if [ "$send_mail" == "Y" ]; then
     ...
     ${KSH_SCRIPTS_DIR}/mail $email   "$cur_proc_name Process Finished Successfully. " $HOME/generic_etl/logs/dummy.txt
