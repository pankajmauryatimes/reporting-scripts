...
echo "Time: $(date +'%d%b%y-%T')"
db2 "select count(1) as total from promotion_data_${curr_proc} pdf where not exists (select 1 from fake_email fe where fe.EMAILID = pdf.EMAIL) and length(pdf.EMAIL) > 0  and length(pdf.CAND_NAME) > 0 and length(pdf.MOBILE) > 0 with ur" > $HOME/generic_etl/wip/data_point_count.txt
...
rm -f $HOME/generic_etl/wip/promotional_data_${curr_proc}_ixf.sql
rm -f $HOME/generic_etl/wip/promotional_data_${curr_proc}_sent_trunc.sql
rm -f $HOME/generic_etl/wip/promotional_data_${curr_proc}_imp.sql
rm -f $HOME/generic_etl/wip/PD_${today}_${curr_proc}.sql
rm -f $HOME/generic_etl/wip/promotional_data_${curr_proc}_sent_del.sql
